...
impl Server {
    ...
    pub fn try_add_primary_camera_display(&mut self) {
        ...
        if !self.contains(&primary_camera_display_name) {
            log::debug!("adding camera service with display index: {}", camera_display::PRIMARY_CAMERA_INDEX);
            self.add_service(Box::new(video_service::new(
                VideoSource::Camera,
                camera_display::PRIMARY_CAMERA_INDEX,
            )));
        }
    }
    ...
    pub fn try_add_primay_video_service(&mut self) {
        let primary_video_service_name =
            video_service::get_service_name(VideoSource::Monitor, *display_service::PRIMARY_DISPLAY_IDX);
